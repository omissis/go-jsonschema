...
// isTryEachCandidate reports whether a `oneOf` qualifies for the try-each
// fallback in either of its forms: object variants dispatched on the top-level
// key-set, or array variants dispatched on their element key-sets. Callers
// should only reach this once detectDiscriminator has declined.
func isTryEachCandidate(variants []*schemas.Type) bool {
	return isTryEachOneOfCandidate(variants) || isTryEachArrayCandidate(variants)
}
