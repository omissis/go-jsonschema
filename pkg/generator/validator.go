...
func (v *stringValidator) generate(out *codegen.Emitter) {
	...
	if v.pattern != "" {
		...
		out.Indent(1)
		out.Println(`return fmt.Errorf("field %%s has invalid pattern: %%s", %s, "%s")`, value, v.pattern)
		out.Indent(-1)
		out.Println(`}`)
		out.Println(`if match := reg.MatchString(%s); !match {`, value)
		out.Indent(1)
		out.Println(`return fmt.Errorf("field %%s does not match pattern: %%s", %s, "%s")`, value, v.pattern)
		out.Indent(-1)
		out.Println(`}`)
	}
