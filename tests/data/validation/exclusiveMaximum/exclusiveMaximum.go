...
func (j *ExclusiveMaximum) UnmarshalJSON(b []byte) error {
	...
	if plain.MyInteger >= 5 {
		return fmt.Errorf("field %s: violates exclusiveMaximum %s", "myInteger", "5")
	}
	if plain.MyNullableInteger != nil && *plain.MyNullableInteger >= 10 {
		return fmt.Errorf("field %s: violates exclusiveMaximum %s", "myNullableInteger", "10")
	}
	if plain.MyNullableNumber != nil && *plain.MyNullableNumber >= 10.3 {
		return fmt.Errorf("field %s: violates exclusiveMaximum %s", "myNullableNumber", "10.3")
	}
	if plain.MyNumber >= 5.6 {
		return fmt.Errorf("field %s: violates exclusiveMaximum %s", "myNumber", "5.6")
	}
